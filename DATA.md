...
| `location` | string | ✅ | Free text. `"remote canada"`, `"Vancouver, Canada"`, `"Remote, US"`, etc. — used by `config.derive_country` (→ `CA`/`IE`/`US`/`OTHER`) and the location pre-filter heuristics. US roles ingest only when `"US" in config.TARGET_COUNTRIES` **and** the role isn't office-bound — a location remote marker passes, else the JD body decides (see `config.location_passes` / `geography.names_office_requirement`). |
| `job_type` | `"remote"` / `"hybrid"` / `"onsite"` / `"unstated"` | ✅ | The resolved work model, from `config.classify_work_model(location, source, claude_work_model)` at ingest. A remote marker in the location (or a `REMOTE_ONLY_SOURCES` board) wins outright via `config.is_remote_role`; otherwise it's Claude's `work_model` read of the JD body from the `score_jd` call, defaulting to `"unstated"`. **Legacy rows** written before the work-model change carry `"unknown"`, which is equivalent to `"unstated"`. US rows are only ever `remote`/`unstated` while `US_ACCEPTED_WORK_MODELS` is at its default — a confirmed `hybrid`/`onsite` US role is discarded at ingest. |
...
| `job_discarded` | `ingest.ingest_job` (various gates) | `"Job discarded: <reason>"` — reasons include missing fields, JD too short, ethics-excluded company, JD refuses sponsorship (skipped for US roles), location not an enabled target geography (US off / office-bound, or remote pinned to a foreign region like "Remote - India"), and **work model** (`"US role is hybrid; accepted: remote/unstated"` — the only gate that fires *after* the Claude scoring call, since the work model is one of its outputs). |
