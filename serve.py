...
CHECKLIST_SECTIONS = [
    ...
    ("cover_letters",   "Cover letters & apply",
     ...
     "as you submit them."),
    ("code_drills",     "Code drills",
     "Interview-prep coding drills. Rebuild fluency writing clean, idiomatic "
     "code by hand under time pressure, then write a test for it."),
]
...
def section_done(sid: str, state: dict, apps_today: int) -> bool:
    ...
    if sid == "code_drills":
        return drills_completed_today() >= DAILY_DRILL_GOAL
    return False
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .app-status-complete         { background: #d4edda; color: #155724; }
...
#
# A daily "Code drills" checklist section. Drills are GENERATED by Claude
# (scripts/drills.py) as short, deliberately underspecified interview-style
# prompts with a partial interface (method names + params, no return types —
# deciding the return shape is part of the drill) and no "watch for" hints. The
# operator implements each Drill<N>.java + Drill<N>Test.java by hand in the
# sibling Maven project (config.MANUAL_CODE_DRILLS_DIR, opened in the editor via
# config.EDITOR_CMD, default "code"), then asks next-role to review the attempt.
# A drills-per-day
# goal (config.DAILY_DRILL_GOAL, default 1) auto-earns the section checkmark,
# mirroring the applications-per-day meter. Store + helpers live in config.py;
# nothing here compiles or runs Java.

_drill_flash: dict | None = None


def set_drill_flash(kind: str, text: str) -> None:
    """kind: 'ok' | 'warn' | 'info'. One-shot flash for the Code drills section."""
    global _drill_flash
    _drill_flash = {"kind": kind, "text": text}


def pop_drill_flash() -> dict | None:
    global _drill_flash
    f, _drill_flash = _drill_flash, None
    return f


def run_drill_command(*args: str) -> tuple[bool, str]:
    """Shell out to scripts/drills.py (generate / review). Returns (ok, output).
    Blocks the request thread for the Claude call (~15-30s), like the inbox scan
    and company-research actions."""
    cmd = [sys.executable, "-u", str(SCRIPTS / "drills.py"), *args]
    try:
        result = subprocess.run(
        ...
        return False, f"Failed to launch drills.py: {e}"
...
def render_drills_body(view: str = "default") -> str:
    ...
    drills  = load_drills()
    done    = drills_completed_today(drills)
    goal    = DAILY_DRILL_GOAL

    # Drills-completed-today meter (mirrors the applications meter).
    met = done >= goal
    parts.append(
        f'<div class="notice {"notice-ok" if met else "notice-info"}" '
    ...
    if not cur:
        parts.append(
            ...
            'prompt — it continues the numbering after the drills already in '
            '<code>manual-code-drills</code>.</p>'
            f'<div class="app-buttons" style="margin-top:10px">{gen_form}{ide_form}</div>')
        return "".join(parts)

    num      = cur.get("number")
    complete = cur.get("status") == "complete"
    status_badge = ('<span class="app-status app-status-complete">complete</span>'
                    if complete else
    ...
    parts.append(
        ...
        f'<code>{esc(str(MANUAL_CODE_DRILLS_DIR))}</code>, then check your work.</p>')

    # Actions: open IDE, review, complete, generate next.
    review_form = (
        '<form method="POST" action="/today/drill/review" style="display:inline">'
        f'<input type="hidden" name="number" value="{num}">'
        '<button class="btn btn-secondary" style="margin-top:0">'
        'Check my code &amp; get feedback</button></form>')
    complete_form = (
        '<form method="POST" action="/today/drill/complete" style="display:inline">'
        f'<input type="hidden" name="number" value="{num}">'
        '<button class="btn btn-secondary" style="margin-top:0">'
    ...
    if fb:
        latest = fb[-1]
        parts.append(
            f'<div style="font-size:13px;margin-top:12px"><strong>Feedback</strong> '
            f'<span style="color:#888">— {esc((latest.get("at") or "")[:16])} · '
...
def render_section_body(sid: str, view: str = "default") -> str:
    ...
    if sid == "code_drills":
        return render_drills_body(view)
...
def daily_checklist_page(open_section: str | None = None, view: str = "default") -> str:
    ...
    for i, (sid, title, hint) in enumerate(CHECKLIST_SECTIONS, 1):
        ...
        if auto_done:
            if sid == "code_drills":
                auto_msg = (f"✓ Auto-completed — {drills_completed_today()} of "
                            f"{DAILY_DRILL_GOAL} drills completed today.")
            else:
                auto_msg = (f"✓ Auto-completed — {apps_today} of "
                            f"{DAILY_APPLICATION_GOAL} applications logged today.")
            toggle_html = (
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/today/drill/generate":
            length = int(self.headers.get("Content-Length", 0))
            params = parse_qs(self.rfile.read(length).decode("utf-8"))
            ok, out = run_drill_command("generate")
            if ok:
                num = next((l.split(":", 1)[1].strip()
                            for l in out.splitlines() if l.startswith("GENERATED:")), "?")
                set_drill_flash("ok", f"Generated Drill {num}.")
            else:
                tail = "; ".join([l for l in out.splitlines() if l.strip()][-2:])
                set_drill_flash("warn", f"Generate failed — {tail or 'see server log'}")
            self.redirect_or_today(params, "code_drills")
            return

        if path == "/today/drill/review":
            length = int(self.headers.get("Content-Length", 0))
            params = parse_qs(self.rfile.read(length).decode("utf-8"))
            number = params.get("number", [""])[0].strip()
            if not number:
                set_drill_flash("warn", "Review failed — missing drill number.")
            else:
                ok, out = run_drill_command("review", "--number", number)
                if ok:
                    set_drill_flash("ok", f"Feedback ready for Drill {number} (below).")
                else:
                    tail = "; ".join([l for l in out.splitlines() if l.strip()][-2:])
        ...
        if path == "/today/drill/complete":
            length = int(self.headers.get("Content-Length", 0))
            params = parse_qs(self.rfile.read(length).decode("utf-8"))
            number = params.get("number", [""])[0].strip()
            if number and mark_drill_complete(int(number)):
                set_drill_flash("ok", f"Drill {number} marked complete.")
            else:
                set_drill_flash("warn", "Complete failed — unknown drill.")
            self.redirect_or_today(params, "code_drills")
            return

        if path == "/today/drill/open-ide":
            import shutil
            length = int(self.headers.get("Content-Length", 0))
            params = parse_qs(self.rfile.read(length).decode("utf-8"))
            folder = str(MANUAL_CODE_DRILLS_DIR)
            if not MANUAL_CODE_DRILLS_DIR.is_dir():
                set_drill_flash("warn", f"Folder not found: {folder}")
            else:
                # Resolve the editor launcher (default "code"). On Windows the VS
                # Code CLI is code.cmd, which CreateProcess can't run directly, so
                # route .cmd/.bat through cmd /c.
                launcher = shutil.which(EDITOR_CMD) or EDITOR_CMD
                if launcher.lower().endswith((".cmd", ".bat")):
                    cmd = ["cmd", "/c", launcher, folder]
                else:
                    cmd = [launcher, folder]
                try:
                    subprocess.Popen(cmd)
                    set_drill_flash("ok", "Opening manual-code-drills in your editor…")
                except Exception as e:  # noqa: BLE001
                    # Editor launch failed — fall back to the OS file manager.
                    try:
                        os.startfile(folder)   # Windows
                        set_drill_flash(
                            "info",
                            f"Editor launch failed ({e}); opened the folder in "
                            "Explorer instead. Set NEXTROLE_EDITOR_CMD if your "
                            "editor CLI isn't 'code'.")
                    except Exception as e2:  # noqa: BLE001
                        set_drill_flash("warn", f"Open failed — {e2}")
            self.redirect_or_today(params, "code_drills")
            return
